...
| `confidence` | 0-1 | 観測専用(Critic にも渡さない) | ダッシュボード |
| `close_pct` | int 10-100 | 決済比率(部分決済可) | execution-plan / Critic 修正可能 / Executor |
| `reasoning` | string | 判断根拠 | ダッシュボード / Critic |
...
| `adjustments.exits` | record<symbol, int 10-100> \| null | exits の **close_pct** 上書き | applyModify → Executor |
