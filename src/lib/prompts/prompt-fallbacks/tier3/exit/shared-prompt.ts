...
- target_price_jpy は **Entry 直後の次サイクル目標** (短期、現在価格 ±数〜十数 %)
...
- ✓ 「Entry 時の仮説が現実と乖離しているか」を評価する材料として使う
  ...
  - 例: Entry 時の短期目標 ¥12,800,000 に対し現在 ¥11,500,000 で下落トレンド → シナリオ崩れ、Close 候補
...
- position_state の建値・含み損益、Analyst の OHLCV はすべて JPY
- 報道由来の USD 価格を **自分の判断根拠の価格水準として使うのは禁止** (¥ 値で評価)
